...
object JDBCUtils extends Logging {
  ...
  def insertStatement(conn: Connection, table: String, rddSchema: StructType, dialect: JdbcDialect)
    ...
    val sql = s"UPSERT USING LOAD INTO $table ($columns) VALUES ($placeholders)"
  ...
  def getSchemaOption(conn: Connection, options: JDBCOptions): Option[StructType] = {
    ...
    try {
      val statement = conn.prepareStatement(dialect.getSchemaQuery(options.parameters(JDBCOptions.JDBC_TABLE_NAME)))
      try {
...
//        println(row.getString(pos))
  ...
  def savePartition(
      ...
      dialect: JdbcDialect,
      isolationLevel: Int,
      sqlStmt:String , //新增的sql语句
      saveMode: CustomSaveMode//新增的写入模式
                   ): Iterator[Byte] = {
...
//      val stmt = insertStatement(conn, table, rddSchema, dialect)
  ...
  def insertStatement(
                          ...
                          dialect: JdbcDialect,
                          mode: CustomSaveMode): String = {
    //如果从数据库中获取不到schema，则从rdd的schema中推到
    val columns = if (tableSchema.isEmpty) {
      rddSchema.fields.map(x => dialect.quoteIdentifier(x.name)).mkString(",")
    } else {
      val tableColumnNames: Array[String] = tableSchema.get.fieldNames
      rddSchema.fields.map { col =>
        val normalizedName = tableColumnNames.find(f => org.apache.spark.sql.catalyst.analysis.caseInsensitiveResolution(f, col.name)).getOrElse {
  ...
  def saveTable(
      ...
      table: String,
      tableSchema: Option[StructType],
      saveMode: CustomSaveMode ,
      options: JDBCOptions) {//TODO JDBCOptions ---> JdbcOptions
    val dialect = JdbcDialects.get(url)
    val nullTypes: Array[Int] = df.schema.fields.map { field =>getJdbcType(field.dataType, dialect).jdbcNullType}
    val rddSchema = df.schema
    //这里通过getInsertStatement方法构造sql语句，接着遍历每个分区进行对应的save操作
    val insertStmt:String = insertStatement(table, rddSchema, tableSchema, dialect, saveMode)
    val getConnection: () => Connection = createConnectionFactory(options)
    val batchSize = options.batchSize
    val isolationLevel = options.isolationLevel
    df.foreachPartition(iterator => savePartition(
      getConnection, table, iterator, rddSchema, nullTypes, batchSize, dialect, isolationLevel,insertStmt,saveMode)
    )
  }
